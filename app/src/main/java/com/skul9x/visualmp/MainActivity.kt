...
class MainActivity : AppCompatActivity() {
        ...
        binding.vpAlbumArt?.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                if (songs != null && position in songs.indices) {
                    ...
                    if (!isUpdateFromService && !isInitialLoad) {
                        musicService?.playSong(songs[position], autoPlay = true)
                    }
    ...
    private fun bindMusicService() {
        ...
        // Ensure service is started to survive background
    ...
    private fun updateUI() {
        musicService?.let { service ->
            // Ensure we have the latest song info logic
            updateSongInfo(service.getCurrentSong())
            // updateSongInfo calls updatePlayerUI internally via onPageSelected ONLY if we snap. 
            // If we are already there, we might need to force update.
            updatePlayerUI()
    ...
    private fun updateSongInfo(song: Song?) {
        ...
        if (songs != null) {
            ...
            if (index != -1 && binding.vpAlbumArt?.currentItem != index) {
                isUpdateFromService = true
                binding.vpAlbumArt?.setCurrentItem(index, true)
                isUpdateFromService = false
                // onPageSelected will fire and call updatePlayerUI()
            } else {
    ...
    private fun showEditMetadataDialog() {
        ...
        dialogBinding.apply {
            ...
            Glide.with(this@MainActivity)
                .load(com.skul9x.visualmp.util.CoverArtFetcher.getCoverArtUri(this@MainActivity, currentSong.id))
                ...
                if (bitmap != null) {
                    ...
                    if (saved) {
                        withContext(Dispatchers.Main) {
                            ...
                            currentEditImageView?.let { iv ->
                                Glide.with(this@MainActivity)
                                    .load(CoverArtFetcher.getCoverArtUri(this@MainActivity, songIdToSave))
