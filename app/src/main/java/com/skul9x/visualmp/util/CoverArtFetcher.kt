...
object CoverArtFetcher {
    ...
    fun downloadImage(context: Context, imageUrl: String): Bitmap? {
        return try {
            ...
            // Download to temporary file first to allow double-decoding (calculate size then decode)
            val tempFile = File.createTempFile("download", "tmp", context.cacheDir)
            ...
            // 2. Calculate inSampleSize (target generic 600x600 or similar)
    ...
    /**
     * Calculate inSampleSize to downsample image to target dimensions
     */
     ...
     */
    fun getCoverArtUri(context: Context, songId: Long): Any {
        ...
        return android.content.ContentUris.withAppendedId(sArtworkUri, songId)
    }
